...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Load walk-forward survivors.
        agg = pl.read_parquet(wf_dir / "rule-validation-aggregate.parquet")
        survivors = agg.filter(pl.col("is_walk_forward_survivor")).sort("mean_val_lift", descending=True)
        print(f"  walk-forward survivors: {survivors.height:,}")
        ...
        for row in sig_df.iter_rows(named=True):
            ...
            # First close any open positions whose exit conditions have been met
            # by signal_date. We process in chronological order.
            still_open: list[Position] = []
            for pos in open_positions:
                exit_info = _check_exit(pos, signal_date, symbol_price_table, args)
            ...
            # Now try to open the new signal.
            ...
            if any(p.symbol == symbol for p in open_positions):
                ...
                continue
            if len(open_positions) >= args.max_concurrent:
            ...
            if fill_open <= 0:
                rejected_no_price += 1
                continue
            entry_price = fill_open * (1.0 + args.slippage_pct)  # slippage hurts on entry
...
def _check_exit(
    ...
    """Check if position should exit by current_date. Returns (exit_date, exit_price, reason) or None."""
    sym_table = symbol_price_table.get(pos.symbol)
    if sym_table is None:
        return None
    ...
    if entry_idx is None:
        return None
    # Walk forward from entry_date+1 to min(current_date, entry+time_decay).
    target = pos.entry_price * (1.0 + args.target_pct)
    stop = pos.entry_price * (1.0 - args.stop_pct)
    time_decay_idx = entry_idx + args.time_decay_days
    for i in range(entry_idx + 1, min(len(sym_table["dates"]), time_decay_idx + 1)):
        d = sym_table["dates"][i]
        if d > current_date:
            break  # haven't reached this day yet
        # Use intraday high/low for stop/target detection.
